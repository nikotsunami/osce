...
{//Assignment E:Module 5
	...
	//spec[
	private void setLunchbreakStart(OsceDayViewImpl osceDayViewImpl,Date lunchBreakStart, int newhr, int newmin) 
	{
		String tempHour=(DateTimeFormat.getFormat("HH:mm").format(lunchBreakStart).substring(0,2));
		String tempMinute=(DateTimeFormat.getFormat("HH:mm").format(lunchBreakStart).substring(3,5));
		
		if(tempHour.compareToIgnoreCase("00")==0 && tempMinute.compareToIgnoreCase("00")==0)
		{
			osceDayViewImpl.getLunchBreakValueLabel().setText(constants.notAvailable());
			osceDayViewImpl.getBtnShiftLunchBreakNext().setEnabled(false);
			osceDayViewImpl.getBtnShiftLunchBreakPrev().setEnabled(false);
		}
		else
		{
			osceDayViewImpl.getLunchBreakValueLabel().setText(DateTimeFormat.getFormat("HH:mm").format(lunchBreakStart).substring(0,5)+"-"+getTimeInTwoDigit(newhr)+":"+getTimeInTwoDigit(newmin));	
		}
...
public static void setOsceFixedButtonStyle(CircuitOsceSubViewImpl circuitOsceSubViewImpl, OsceProxy osceProxy){
	...
	//sequence start
		...
		// E Module 5 and TTG Bug Changes
		{
			...
			RoomRefreshEvent.register(requests.getEventBus(), (OscePostSubViewImpl)oscePostSubView);
			// Change in ParcourView
			//spec issue sol
			if(oscePostProxy.getStandardizedRole() != null)
			{
				...
				// E Module 5 Bug Test Change
		 	        ...
		 	        oscePostSubView.getStandardizedRoleLbl().setText(constants.role()+": ");

			oscePostSubView.getRoomLbl().setText(constants.room()+": ");
			// E Change in ParcourView
				...
				if(oscePostProxy.getOscePostBlueprint().getSpecialisation()!=null)
				{
				...
				if(oscePostProxy.getOscePostBlueprint().getRoleTopic()!=null)
				{
			...
			if(oscePostProxyNext.getStandardizedRole() != null)
			{
				...
				// E Module 5 Bug Test Change
		         	...
		         	oscePostSubViewNext.getStandardizedRoleLbl().setText(constants.role()+": ");
			oscePostSubViewNext.getRoomLbl().setText(constants.room()+": ");
			// E Change in ParcourView
			...
			if(oscePostProxyNext.getOscePostBlueprint().getSpecialisation()!=null)
			{
			...
			if(oscePostProxyNext.getOscePostBlueprint().getRoleTopic()!=null)
			{
		...
		// E Module 5 and TTG Bug Changes
		{
			...
			{
				oscePostSubView.getStandardizedRoleLbl().setVisible(false);
				oscePostSubView.getRoomLbl().setVisible(false);
				oscePostSubView.getStandardizedRoleEdit().setVisible(false);
				oscePostSubView.getRoomedit().setVisible(false);
			}
		...
		// E Module 5 and TTG Bug Changes
		{
			...
			if(oscePostProxy.getOscePostBlueprint().getRoleTopic()!=null)
			{
				oscePostSubView.getRoleTopicLbl().setText(util.getEmptyIfNull(getLabelString(oscePostProxy.getOscePostBlueprint().getRoleTopic().getName())));			
...
//													oscePostBlueprintProxyNew.setIsPossibleStart(null);
												...
												Log.info("Save OscePost Bluprint");
...
//oscePostSubViewImpl.get(innerindex).setDelegate(circuitDetailsActivity); // SET DELEGATE FOR SUBVIEW						
					...
					if(oscePostBlueprintProxy.getPostType()==PostType.ANAMNESIS_THERAPY || oscePostBlueprintProxy.getPostType()==PostType.PREPARATION)
					{
						Log.info("~Anemnis");				
							...
							tempOscePostSubViewImplNext.getSpecializationLbl().setText(getLabelString(oscePostBlueprintProxyNext.getSpecialisation()==null?constants.discipline()+": ":oscePostBlueprintProxyNext.getSpecialisation().getName()));
							tempOscePostSubViewImplNext.getSpecializationLbl().setTitle(oscePostBlueprintProxyNext.getSpecialisation()==null?constants.discipline()+": ":oscePostBlueprintProxyNext.getSpecialisation().getName());
							tempOscePostSubViewImplNext.getRoleTopicLbl().setText(getLabelString(oscePostBlueprintProxyNext.getRoleTopic()==null?constants.topic()+": ":oscePostBlueprintProxyNext.getRoleTopic().getName()));
							tempOscePostSubViewImplNext.getRoleTopicLbl().setTitle(oscePostBlueprintProxyNext.getRoleTopic()==null?constants.topic()+": ":oscePostBlueprintProxyNext.getRoleTopic().getName());
				...
				public void specializationEditClicked(final OscePostSubViewImpl oscePostSubViewImpledit) 
				{
					...
					requests.specialisationRequest().findSpecialisationSortByName(osceProxy.getStudyYear()).fire(new OSCEReceiver<List<SpecialisationProxy>>() 
					{
						...
						{											
							...
							((ListBoxPopupViewImpl)((OscePostSubViewImpl)oscePostSubViewImpledit).popupView).getNewListBox().setSelected(((OscePostSubViewImpl)oscePostSubViewImpledit).getSpecialisationProxy());
				...
				public void roleEditClicked(final OscePostSubViewImpl oscePostSubViewImpledit) 
				{
					...
					requests.roleTopicRequest().findRoleTopicBySpecialisation(oscePostSubViewImpledit.oscePostBlueprintProxy.getSpecialisation().getId(), oscePostSubViewImpledit.oscePostBlueprintProxy).fire(new OSCEReceiver<List<RoleTopicProxy>>() 
					{
						...
						public void onSuccess(List<RoleTopicProxy> response) 
						{
							...
							((OscePostSubViewImpl)oscePostSubViewImpledit).listBoxPopupViewImpl.getNewListBox().setSelected(((OscePostSubViewImpl)oscePostSubViewImpledit).getRoleTopicProxy());
							...
							//Issue # 122 : Replace pull down with autocomplete.
							((OscePostSubViewImpl)oscePostSubViewImpledit).listBoxPopupViewImpl.getNewListBox().setDefaultText("");
						}
					});
				...
				//	final SpecialisationProxy specialisationProxy=(SpecialisationProxy)oscePostSubViewImplok.getListBoxPopupViewImpl().getListBox().getValue();
					...
					oscePostBlueprintRequest.persist().using(oscePostBlueprintProxy).fire(new OSCEReceiver<Void>(oscePostSubViewImplok.oscePostBluePrintMap)
					{
						...
						public void onSuccess(Void response) 
						{
							...
							setProxy(oscePostSubViewImplok, specialisationProxy);
							setProxy(oscePostSubViewImplok, null);
							...
							oscePostSubViewImplok.getRoleTopicLbl().setText(constants.topic()+":");
							oscePostSubViewImplok.listBoxPopupViewImpl.hide();	
						}
				...
				public void saveRoleTopic(final OscePostSubViewImpl oscePostSubViewImplok) 
				{
					...
					oscePostBlueprintRequest.persist().using(oscePostBlueprintProxy).fire(new OSCEReceiver<Void>(oscePostSubViewImplok.oscePostBluePrintMap)
					{
						...
						public void onSuccess(Void response) 
						{
							...
							setProxy(oscePostSubViewImplok, roleTopicProxy);
...
*/										
				...
				@Override
				public void closeButtonClicked(final OsceProxy proxy) {
					...
					requests.oscePostRequest().findOscePostByOsce(proxy.getId()).fire(new OSCEReceiver<List<OscePostProxy>>() 
					{
						...
						public void onSuccess(List<OscePostProxy> response) 
						{
							...
							{																	
								...
								requests.getEventBus().fireEvent(new ApplicationLoadingScreenEvent(false));
								// E Module 5 Bug Test Change
								MessageConfirmationDialogBox roleNullWarning=new MessageConfirmationDialogBox(constants.warning());
								roleNullWarning.showConfirmationDialog(constants.warningStandardizedRoleUnassigned());								
							}
							else
							{
				...
				{					
					...
					requests.roomRequest().findAllRoomsOrderByRoomNumber().fire(new OSCEReceiver<List<RoomProxy>>() 
					{
							...
							public void onSuccess(List<RoomProxy> response) 
							{
								...
								OscePostSubViewImpl opsv=new OscePostSubViewImpl();
								opsv=((OscePostSubViewImpl)oscePostSubView);
								ArrayList list=new ArrayList();
								//list.add( (EntityProxy)response.getStandardizedRoles().get(0));
								list.addAll(response);
								
								opsv.createOptionPopup();
								//((OscePostSubViewImpl)oscePostSubView).popupView.getNewListBox().setDefaultText(constants.select());
								opsv.popupView.setDelegate(activity);
								opsv.popupView.setOscePostSubView(oscePostSubView);
								opsv.popupView.setProxy(oscePostSubView.getOscePostProxy());								
								opsv.showPopUpViewForRoom(left,top);								
								//opsv.popupView.getNewListBox().setDefaultText(" ");
								//Issue # 122 : Replace pull down with autocomplete.
								DefaultSuggestOracle<Object> suggestOracle1 = ((DefaultSuggestOracle<Object>) (opsv.popupView.getNewListBox().getSuggestOracle()));
								suggestOracle1.setPossiblilities(list);
								opsv.popupView.getNewListBox().setSuggestOracle(suggestOracle1);									
								//((OscePostSubViewImpl)view).popupView.getNewListBox().setRenderer(new StandardizedRoleProxyRenderer());
								opsv.popupView.getNewListBox().setRenderer(new Renderer<Object>() 
								{
											...
											else
											{
												...
												//{
													...
													dialog.getYesBtn().addClickHandler(new ClickHandler() {
														...
														{
															dialog.hide();
															exchangeRooms(oscePostSubViewImpl.getCourseProxy(), oscePostSubViewImpl.getOscePostProxy(),view,roomProxy,oscePostRoom,oscePostSubViewImpl,view);
		...
		/*			OscePostProxy oscePostProxy=(OscePostProxy)view.getProxy();
				...
				protected void replaceRooms(final OscePostRoomProxy selectedRoom,OscePostRoomProxy newOscePostRoomProxy,final long osceId, final OscePostSubViewImpl oscePostSubViewImpl, final ListBoxPopupView view) 
				{
					...
					requests.oscePostRoomRequest().findOscePostRoom(newOscePostRoomProxy.getId()).with("room","oscePost","course").fire(new OSCEReceiver<OscePostRoomProxy>() {
						...
						@Override
						public void onSuccess(OscePostRoomProxy newOscePostRoom) 
						{						
							final RoomProxy oldRoomProxy=selectedRoom.getRoom();
							final RoomProxy newRoomProxy=newOscePostRoom.getRoom();
	...
	public void refreshRoomValue(final OscePostSubViewImpl oscePostSubViewImpl, String roomValue) {
		...
		/*requests.oscePostRoomRequest().findOscePostRoomByOscePostAndCourse(courseProxy, oscePostProxy).with("room").fire(new OSCEReceiver<OscePostRoomProxy>() {
			...
			public void onSuccess(OscePostRoomProxy response) {
				if (response.getRoom() != null)
				{				
					oscePostSubViewImpl.getRoomLbl().setText(util.getEmptyIfNull(response.getRoom().getRoomNumber()));
				}

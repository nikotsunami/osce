...
public class SPFederalAllocationUpdator {
	...
	public SPFederalAllocationUpdator(OsceDay osceDay) {		
	...
	public void allocateSp()
	{
			...
			for (OsceSequence osceSequence : osceDay.getOsceSequences())
			{
				...
				for (OscePost oscePost : oscePostList)
				{
					...
					//All PIR of OSCE sequence and OSCE post order by isAscending.
					...
					List<PatientInRole> assignmentSPList = PatientInRole.findAllSPFromAssignment(osceDay.getId(), oscePost.getId());
					
					patientInRoleList.removeAll(assignmentSPList);
					//All new SPS that is not assigned in assignment.
					List<PatientInRole> allNewAssignPatientOfPost = PatientInRole.findAllNewAssignedPatientOfPost(osceDay.getId(),oscePost.getId(), patientInRoleList);
					...
					if (PostType.DUALSP.equals(oscePost.getOscePostBlueprint().getPostType()))
					{
						...
						for (PatientInRole pir : allNewAssignPatientOfPost)
						{
					...
					//findAllEmptyAssignmentOfOscePostAndOsceDayByBreakAssignmentFirst
					...
					for (Assignment ass : postAssList)
					{
						...
						if (PostType.DUALSP.equals(oscePost.getOscePostBlueprint().getPostType()))
							...
							assList = Assignment.findAllEmptyAssignmentOfOscePostAndOsceDay(osceDay.getId(), oscePost.getId(), ass.getTimeStart(), ass.getTimeEnd(), ass.getSequenceNumber());
